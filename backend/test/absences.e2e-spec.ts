...
describe('Congés / absences (e2e)', () => {
  ...
  async function setupEmployee() {
    ...
    const login = await request(http)
      ...
      .expect(201);
    return {
      empToken: login.body.token as string,
      ficheId: fiche.body.id as string,
    };
  }
  ...
  it('flux complet : demande employé → en attente → validation owner → solde mis à jour', async () => {
    ...
    await request(http)
      .patch(`/absences/${req.body.id}/status`)
      .set(owner())
      .send({ status: 'approved' })
      .expect(200);

    // Le solde est mis à jour côté employé.
    me = await request(http)
      .get('/employees/me')
      .set(emp(empToken))
      .expect(200);

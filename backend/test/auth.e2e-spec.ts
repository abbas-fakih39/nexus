...
describe('Auth & RBAC (e2e)', () => {
  ...
  it('login avec un mauvais mot de passe → 401', async () => {
    ...
    await request(http).post('/auth/login').send({ email: 'o@test.fr', password: 'wrong' }).expect(401);
  });

  it('login owner OK puis /auth/me renvoie le profil sans mot de passe', async () => {
    const token = await registerOwner(app, 'o@test.fr');
    const me = await request(http).get('/auth/me').set('Authorization', `Bearer ${token}`).expect(200);
  ...
  it('un employé ne peut pas lister les employés (403) mais voit les produits (200)', async () => {
    ...
    await request(http).get('/employees').set('Authorization', `Bearer ${empToken}`).expect(403);
    await request(http).get('/products').set('Authorization', `Bearer ${empToken}`).expect(200);
  });
  ...
  it('un compte désactivé ne peut plus se connecter (401)', async () => {
    ...
    await request(http).post('/auth/login').send({ email: 'emp@test.fr', password: 'secret123' }).expect(401);
  });
});

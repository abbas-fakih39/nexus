...
describe('AbsencesService', () => {
  ...
  describe('create', () => {
    ...
    it('owner : exige un employeeId', async () => {
      await expect(
        service.create({ ...base }, { id: 'o', role: 'owner' }),
      ).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('employé : 400 sans fiche associée', async () => {
      prisma.employee.findUnique.mockResolvedValue(null);
      await expect(
        service.create({ ...base }, { id: 'u1', role: 'employee' }),
      ).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('respecte un nombre de jours fourni (demi-journée)', async () => {
      prisma.employee.findUnique.mockResolvedValue({ id: 'e1' });
      await service.create(
        { ...base, employeeId: 'e1', days: 0.5 },
        { id: 'o', role: 'owner' },
      );
      expect(prisma.absence.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ days: 0.5 }),
        }),
      );
    });
  });
  ...
  describe('decide', () => {
    it('refuse une décision « pending »', async () => {
      await expect(
        service.decide('a1', AbsenceStatus.pending),
      ).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('404 si l’absence n’existe pas', async () => {
      ...
      await expect(
        service.decide('a1', AbsenceStatus.approved),
      ).rejects.toBeInstanceOf(NotFoundException);
    });
  ...
  describe('remove', () => {
    it('404 si introuvable', async () => {
      prisma.absence.findUnique.mockResolvedValue(null);
      await expect(
        service.remove('a1', { id: 'o', role: 'owner' }),
      ).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('owner : supprime n’importe quelle absence', async () => {
      prisma.absence.findUnique.mockResolvedValue({
        id: 'a1',
        status: AbsenceStatus.approved,
        employee: { userId: 'someone' },
      });
      await service.remove('a1', { id: 'o', role: 'owner' });
      expect(prisma.absence.delete).toHaveBeenCalledWith({
        where: { id: 'a1' },
      });
    });
    ...
    it('employé : 403 si ce n’est pas sa fiche', async () => {
      prisma.absence.findUnique.mockResolvedValue({
        id: 'a1',
        status: AbsenceStatus.pending,
        employee: { userId: 'autre' },
      });
      await expect(
        service.remove('a1', { id: 'u1', role: 'employee' }),

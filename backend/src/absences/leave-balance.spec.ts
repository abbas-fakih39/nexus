...
describe('computeLeaveBalance', () => {
  ...
  it('compte les congés payés approuvés dans « pris » et réduit le restant', () => {
    const r = computeLeaveBalance(25, [abs(AbsenceType.paid_leave, AbsenceStatus.approved, 5)], YEAR);
  ...
  it('compte les congés payés en attente sans toucher au restant', () => {
    const r = computeLeaveBalance(25, [abs(AbsenceType.paid_leave, AbsenceStatus.pending, 3)], YEAR);
  ...
  it('ignore les demandes refusées', () => {
    const r = computeLeaveBalance(25, [abs(AbsenceType.paid_leave, AbsenceStatus.rejected, 4)], YEAR);
    expect(r.taken).toBe(0);
    expect(r.pending).toBe(0);
    expect(r.remaining).toBe(25);
  });
  ...
  it('ignore les types autres que congé payé (maladie, sans solde…)', () => {
    const r = computeLeaveBalance(25, [
  ...
  it('ne compte que les absences de l’année demandée', () => {
    const r = computeLeaveBalance(25, [
      abs(AbsenceType.paid_leave, AbsenceStatus.approved, 5, new Date('2025-03-02')),
      abs(AbsenceType.paid_leave, AbsenceStatus.approved, 2, new Date('2026-03-02')),
    ], YEAR);
  ...
  it('accepte les jours sérialisés en chaîne (Decimal)', () => {
    const r = computeLeaveBalance(25, [
      { type: AbsenceType.paid_leave, status: AbsenceStatus.approved, days: '5', startDate: new Date('2026-03-02') },
    ], YEAR);
    expect(r.taken).toBe(5);
  });
});

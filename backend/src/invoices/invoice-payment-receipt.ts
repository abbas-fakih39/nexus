...
export function buildPaymentReceipt(
  ...
  const amount = isSale ? num(invoice.sale?.finalAmount) : num(invoice.purchase?.totalAmount);
  ...
  if (isSale && invoice.sale) {
    row('Réglé par', PAYMENT_LABEL[invoice.sale.paymentMethod] ?? invoice.sale.paymentMethod, 'Courier', 8, MUTE);
    if (invoice.sale.clientName) row('Reçu de', invoice.sale.clientName, 'Courier', 8, MUTE);
  ...
  center(isSale ? 'Merci de votre confiance' : 'Conservez ce reçu', 'Courier', 7.5, MUTE, 1);

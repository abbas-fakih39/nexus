...
export default function Comptes() {
  ...
  async function handleCreate(e: FormEvent) {
    ...
    if (form.password.length < 6) return setFormError('Le mot de passe doit faire au moins 6 caractères.');
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-ink-mute">
          Créez des accès pour vos employés. Un compte désactivé ne peut plus se connecter.
        </p>
        <Button onClick={openAdd} icon={<PlusIcon />}>Ajouter un compte</Button>
      </div>

      {rowError && (
        <div role="alert" className="rounded-xl border border-danger/30 bg-danger-soft px-4 py-3 text-sm font-medium text-danger">
          {rowError}
        </div>
      )}
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
                ...
                return (
                  <tr key={a.id} className="border-b border-border last:border-0 hover:bg-canvas">
                    <td className="px-5 py-3">
                      <div className="font-semibold text-ink">{a.name}{isSelf && <span className="ml-2 text-[11px] font-medium text-ink-faint">(vous)</span>}</div>
                      <div className="text-[12px] text-ink-mute">{a.email}</div>
                    </td>
                    <td className="px-5 py-3">
                      <Badge tone={isOwner ? 'neutral' : 'success'}>{isOwner ? 'Propriétaire' : 'Employé'}</Badge>
                    </td>
                    <td className="px-5 py-3">
                      <Badge tone={a.isActive ? 'success' : 'danger'}>{a.isActive ? 'Actif' : 'Désactivé'}</Badge>
                    </td>
                    <td className="px-5 py-3 text-ink-soft">
                      {ops === 0 ? <span className="text-ink-faint">—</span> : `${a._count.sales} ventes · ${a._count.purchases} achats`}
                    </td>
                    <td className="px-5 py-3 font-mono tabular-nums text-ink-mute">{dateFr(a.createdAt)}</td>
                    <td className="px-5 py-3">
        ...
        {accounts.length === 0 && (
          <div className="px-5 py-14 text-center text-sm text-ink-mute">Aucun compte.</div>
        )}
      </div>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button type="submit" form="account-form" loading={saving}>Créer le compte</Button>
          </>
        }
      >
        <form id="account-form" onSubmit={handleCreate} className="flex flex-col gap-4">
          {formError && (
          ...
          <Input label="Nom complet *" value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="Ex. Sofiane Benali" autoFocus />
          <Input label="Email *" type="email" value={form.email} onChange={(e) => set('email', e.target.value)} placeholder="employe@maboutique.fr" />
          <Input label="Mot de passe *" type="password" value={form.password} onChange={(e) => set('password', e.target.value)} placeholder="6 caractères minimum" />
          <p className="text-[12px] text-ink-faint">L'employé se connecte avec cet email et ce mot de passe. Il aura un accès limité (ventes, stock en lecture).</p>
        </form>
      </Modal>
      ...
      >
        ...
        <p className="text-sm text-ink-soft">
          Confirmer la suppression du compte de <span className="font-semibold text-ink">{deleteTarget?.name}</span> ?
          {deleteTarget && deleteTarget._count.sales + deleteTarget._count.purchases > 0 && (
            <span className="mt-2 block text-[12px] text-ink-mute">
              Ce compte a un historique d'opérations : la suppression sera refusée. Préférez la désactivation.
            </span>
          )}
        </p>
      </Modal>

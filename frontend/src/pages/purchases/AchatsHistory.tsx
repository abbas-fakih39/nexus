...
const STATUS_BADGE: Record<
  PurchaseStatus,
  { tone: "success" | "danger"; label: string }
> = {
...
export default function AchatsHistory() {
  ...
  async function load() {
    ...
    try {
      setPurchases(
        await getPurchases({ from: from || undefined, to: to || undefined }),
      );
    } finally {
      setLoading(false);
    }
  ...
  const { page, pageCount, pageItems, total, pageSize, setPage } =
    usePagination(filtered, 10, `${status}|${search}|${from}|${to}`);
  ...
  async function confirmCancel() {
    ...
    try {
      ...
      setCancelError(
        Array.isArray(m)
          ? m.join(", ")
          : (m ?? "Impossible d'annuler cet achat."),
      );
    } finally {
      setCancelling(false);
    }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          ...
          tone={stats.cancelled ? "danger" : undefined}
        />
        <StatCard
          label="Total dépensé"
          value={formatEuro(stats.spent)}
          tone="accent"
        />
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table
              aria-label="Historique des achats"
                ...
                {pageItems.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-3 text-right font-mono text-ink-soft tabular-nums">
                        {p._count?.items ?? "—"}
                      </td>
                      <td
                        className={`px-5 py-3 text-right font-mono font-semibold tabular-nums ${p.status === "cancelled" ? "text-ink-faint line-through" : "text-ink"}`}
                      >
          ...
          detail ? (
            <>
              ...
              <div className="ml-auto flex items-center gap-3">
                {cancelError && (
                  <span className="text-[13px] text-danger">{cancelError}</span>
                )}
                <Button
                  variant="secondary"
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="font-mono text-[13px] text-ink-mute tabular-nums">
          ...
          <div className="mt-0.5 text-[13px] text-ink-mute">
            Créé par {detail.createdBy?.name ?? "—"}
          </div>
        </div>
        <Badge tone={badge.tone}>{badge.label}</Badge>
      </div>
      ...
      {detail.notes && (
        <div className="rounded-xl bg-canvas px-3 py-2 text-[13px] text-ink-soft">
          <span className="font-semibold text-ink-mute">Notes : </span>
          {detail.notes}
        </div>
      )}
                ...
                >
                  <td className="px-4 py-2.5">
                    <div className="font-medium text-ink">
                      {it.product?.name ?? "Produit supprimé"}
                    </div>
                    {it.product?.sku && (
...
function StatCard({
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface px-5 py-4 shadow-sm">
      ...
      <div
        className={`mt-1 font-mono text-2xl font-bold tabular-nums ${valueColor}`}
      >
        {value}
      </div>
    </div>

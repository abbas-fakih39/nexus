...
export default function NouvelAchat() {
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort(
      (a, b) =>
        (a.supplierId === supplierId ? 0 : 1) -
        (b.supplierId === supplierId ? 0 : 1),
    );
  ...
  function addToCart(p: Product) {
    setCart((c) => {
      ...
      return [
        ...c,
        { product: p, quantity: 1, unitCost: Number(p.costPrice) || 0 },
      ];
    });
  }
  ...
  function setQty(id: string, qty: number) {
    setCart((c) =>
      c.map((l) =>
        l.product.id === id ? { ...l, quantity: Math.max(1, qty) } : l,
      ),
    );
  }
  function setUnitCost(id: string, cost: number) {
    setCart((c) =>
      c.map((l) =>
        l.product.id === id ? { ...l, unitCost: Math.max(0, cost) } : l,
      ),
    );
  }
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_minmax(360px,400px)]">
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Pill active={categoryId === ""} onClick={() => setCategoryId("")}>
            Tous
          </Pill>
          {categories.map((c) => (
            <Pill
        ...
        <div className="grid grid-cols-2 gap-3 xl:grid-cols-3">
          {filtered.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`flex flex-col rounded-xl border bg-surface p-3.5 shadow-sm ${habitual ? "border-accent/40" : "border-border"}`}
              >
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <span
                    className={`text-[11.5px] font-semibold ${out ? "text-danger" : low ? "text-warn" : "text-ink-mute"}`}
                  >
                    {out
                      ? "Rupture — à réapprovisionner"
                      : low
                        ? `${p.stock} en stock (faible)`
                        : `${p.stock} en stock`}
                  </span>
                  {habitual && (
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <Button
                    size="sm"
                    variant={inCart(p.id) ? "secondary" : "primary"}
                    onClick={() => addToCart(p)}
                  >
          ...
          {filtered.length === 0 && (
            <div className="col-span-full py-12 text-center text-sm text-ink-mute">
              Aucun produit.
            </div>
          )}
        </div>
      ...
      <aside className="flex h-fit flex-col gap-4 rounded-2xl border border-border bg-surface p-5 shadow-sm lg:sticky lg:top-[88px]">
        ...
        {error && (
          <div
            role="alert"
            className="rounded-xl border border-danger/30 bg-danger-soft px-3 py-2 text-[13px] font-medium text-danger"
          >
            {error}
          </div>
        )}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5 flex items-center gap-1.5">
                    ...
                    <input
                      ...
                      value={l.unitCost || ""}
                      onChange={(e) =>
                        setUnitCost(l.product.id, Number(e.target.value))
                      }
                      placeholder="0.00"
      ...
      <Modal
        ...
        footer={
          <div className="flex w-full items-center justify-between gap-3">
            {success?.invoiceId ? (
              <InvoiceActions invoiceId={success.invoiceId} />
            ) : (
              <span />
            )}
            <Button onClick={() => setSuccess(null)}>Nouvel achat</Button>
          </div>
        }
      >
        <div className="flex flex-col gap-2 text-sm">
          <p className="text-ink-soft">
            Le stock a été mis à jour et le coût d'achat des produits
            enregistré.
          </p>
          <div className="flex justify-between border-t border-border pt-2">
            <span className="text-ink-mute">
              Montant ({success?.items} ligne
              {(success?.items ?? 0) > 1 ? "s" : ""})
            </span>
            <span className="font-mono font-bold tabular-nums text-ink">
              {formatEuro(success?.total ?? 0)}
            </span>
          </div>
        </div>
      </Modal>
    ...
    <button
      ...
      className={`rounded-full border px-3.5 py-1.5 text-[12.5px] font-semibold transition ${
        active
          ...
          : "border-border bg-surface text-ink-mute hover:bg-canvas"
      }`}
    >
      {children}
    </button>
  ...
  return (
    <div className="inline-flex items-center rounded-lg border border-border">
      <button
        ...
        className="grid h-7 w-7 place-items-center text-ink-mute transition hover:text-ink disabled:opacity-40"
        disabled={value <= 1}
      >
        <MinusIcon />
      </button>
      <span className="w-7 text-center font-mono text-[13px] font-semibold tabular-nums text-ink">
        {value}
      </span>
      <button
        type="button"
        ...
        onClick={() => onChange(value + 1)}
        className="grid h-7 w-7 place-items-center text-ink-mute transition hover:text-ink"
      >
        <PlusIcon />
      </button>
    </div>

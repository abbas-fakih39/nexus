...
export default function FichesEmployes() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!form.firstName.trim() || !form.lastName.trim())
      return setFormError("Le nom et le prénom sont obligatoires.");
    if (!form.jobTitle.trim()) return setFormError("Le poste est obligatoire.");
    const salary = Number(form.baseSalary);
    if (!form.baseSalary || Number.isNaN(salary) || salary < 0)
      return setFormError("Salaire de base invalide.");
    if (!form.hiredAt)
      return setFormError("La date d'embauche est obligatoire.");
    const quota = Number(form.leaveQuota);
    if (form.leaveQuota === "" || !Number.isInteger(quota) || quota < 0)
      return setFormError("Quota de congés invalide.");
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-ink-mute">
          Gérez les fiches du personnel et rattachez-les éventuellement à un
          compte de connexion.
        </p>
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
        <div className="overflow-x-auto">
          <table aria-label="Fiches employés" className="w-full text-sm">
            <thead>
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <div className="flex justify-end gap-1">
                      <IconButton label="Modifier" onClick={() => openEdit(e)}>
                        <PencilIcon />
                      </IconButton>
                      <IconButton
                        label="Supprimer"
      ...
      >
        ...
        >
          ...
          <div>
            <Select
              label="Compte de connexion (optionnel)"
              value={form.userId}
            ...
            <p className="mt-1.5 text-[12px] text-ink-faint">
              Choisir un compte importe automatiquement son nom. Les comptes se
              créent dans Paramètres → Comptes.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            />
            <Input
              label="Congés payés (j/an) *"
              type="number"
              min="0"
              step="1"
              value={form.leaveQuota}
              onChange={(e) => set("leaveQuota", e.target.value)}
              placeholder="25"
            />
          </div>
        </form>
      ...
      >
        ...
        <p className="text-sm text-ink-soft">
          Confirmer la suppression de la fiche de{" "}
          ...
          {deleteTarget && deleteTarget._count.payments > 0 && (
            <span className="mt-2 block text-[12px] text-ink-mute">
              Cette fiche a des paiements de salaire enregistrés : la

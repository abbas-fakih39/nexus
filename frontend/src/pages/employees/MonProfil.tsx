  ...
  async function submitRequest(e: FormEvent) {
    ...
    if (form.mode === "single") {
      ...
      if (!form.startDate || !form.endDate)
        return setFormError("Renseignez les dates.");
      if (form.endDate < form.startDate)
        return setFormError("La date de fin doit être après le début.");
      startDate = form.startDate;
      endDate = form.endDate;
    }
    if (form.type === "other" && !form.reason.trim()) {
      return setFormError(
        "Le motif est obligatoire pour une absence de type « Autre ».",
      );
    }
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-mute">
          Votre fiche, vos congés, absences et salaires.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-surface p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="rounded-xl bg-accent-softer px-4 py-2 text-right">
            <div className="text-[11px] font-medium text-accent-deep/80">
              Salaire de base
            </div>
            <div className="font-mono text-lg font-bold tabular-nums text-accent-deep">
              {formatEuro(me.baseSalary)}
            </div>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-surface p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Button onClick={openRequest} icon={<PlusIcon />}>
            Demander un congé
          </Button>
        </div>
      ...
      <Section title="Mes absences">
        <Table
          label="Mes absences"
          head={["Type", "Période", "Jours", "Statut", "Motif", ""]}
        >
          {me.absences.map((a) => (
            <tr
              key={a.id}
              ...
              <td className="px-5 py-3 text-right">
                {a.status === "pending" && (
                  <button
                    ...
                    className="rounded-lg px-2.5 py-1.5 text-[12px] font-semibold text-danger transition hover:bg-danger-soft disabled:opacity-50"
                  >
                    Annuler
                  </button>
                )}
      ...
      <Section
        title="Mes retards"
        subtitle={`${me.counters.tardinessCount} cette année · ${me.counters.tardinessMinutes} min cumulées`}
      >
        <Table
          label="Mes retards"
          head={["Date", "Retard", "Justifié", "Note"]}
        >
          {me.tardiness.map((t) => (
            <tr
              key={t.id}
      ...
      <Section
        ...
        subtitle={`${me.counters.overtimeHours} h cette année`}
      >
        <Table
          label="Mes heures supplémentaires"
          head={["Date", "Heures", "Note"]}
        >
          {me.overtimes.map((o) => (
            <tr
              key={o.id}
        ...
        {me.overtimes.length === 0 && (
          <Empty text="Aucune heure supplémentaire." />
        )}
      </Section>
      ...
      <Section title="Historique des salaires">
        <Table
          ...
          head={["Mois", "Montant", "Note", "Payé le"]}
        >
          {me.payments.map((p) => (
            <tr
              key={p.id}
            ...
            >
              ...
              <td className="px-5 py-3 font-mono tabular-nums font-semibold text-ink">
                {formatEuro(p.amount)}
              </td>
        ...
        {me.payments.length === 0 && (
          <Empty text="Aucun salaire enregistré pour le moment." />
        )}
      </Section>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button type="submit" form="leave-form" loading={saving}>
              Envoyer la demande
            </Button>
          </>
        }
      >
        ...
        >
          ...
          <Select
            ...
            value={form.type}
            onChange={(e) =>
              setForm((f) => ({ ...f, type: e.target.value as AbsenceType }))
            }
            autoFocus
          >
            {TYPES.map((t) => (
          ...
          <Input
            ...
            placeholder={
              form.type === "other"
                ? "Obligatoire pour une absence « Autre »"
                : "Optionnel"
            }
          />
          <p className="text-[12px] text-ink-faint">
            Votre demande sera transmise au gérant pour validation.
          </p>
        </form>
      </Modal>
    </div>
...
function Field({
  ...
  return (
    <div>
      ...
      <dd
        className={`mt-1 text-[15px] font-semibold text-ink ${mono ? "font-mono tabular-nums" : ""}`}
      >
        {value}
      </dd>
    </div>
...
function Stat({
  ...
  return (
    <div className="rounded-xl border border-border bg-canvas px-4 py-3">
      ...
      <div
        className={`mt-0.5 font-mono text-xl font-bold tabular-nums ${color}`}
      >
        {value}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-baseline justify-between gap-3">
        ...
        {subtitle && (
          <span className="text-[12px] text-ink-mute">{subtitle}</span>
        )}
      </div>

...
const DAYS_FOR: Record<Period, number> = {
  today: 7,
  "7d": 7,
  "30d": 30,
  year: 90,
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <Kpi
              ...
              accent
              trend={pct(stats.revenue, stats.prev.revenue)}
            />
            <Kpi
              label="Marge"
              value={formatEuro(stats.margin)}
              hint={`${stats.marginRate}% du CA`}
              trend={pct(stats.margin, stats.prev.margin)}
            />
            <Kpi
              label="Ventes"
              value={String(stats.salesCount)}
              hint={`Panier moy. ${formatEuro(stats.avgBasket)}`}
              trend={pct(stats.salesCount, stats.prev.salesCount)}
            />
            <Kpi
              label="Valeur du stock"
              value={formatEuro(stats.stockValue)}
              hint={`${stats.lowStockCount} alerte(s)`}
              hintWarn={stats.lowStockCount > 0}
            />
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <ChartCard
              ...
              subtitle={`${DAYS_FOR[period]} derniers jours`}
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <div className="truncate text-[11.5px] text-ink-faint">
                            {p.category?.name ?? "—"} · seuil {p.alertThreshold}
                          </div>
                        </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <Panel
              ...
              hint="en stock, 0 vente sur la période"
            >
              {dormant.length === 0 ? (
                  ...
                  {dormant.slice(0, 6).map((p) => (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <div className="truncate text-[11.5px] text-ink-faint">
                          {p.category?.name ?? "—"} · {p.stock} {p.unit}
                        </div>
                      </div>
                      <span className="shrink-0 font-mono text-[12.5px] font-semibold tabular-nums text-ink-mute">
                        {formatEuro(p.value)}
                      </span>
                    </li>
                  ))}
...
function MoneyTooltip({
  ...
  label,
  nameKey,
}: TooltipPayload & { nameKey?: string }) {
  if (!active || !payload?.length) return null;
  const row = payload[0];
  const title = nameKey
    ? String(row.payload[nameKey])
    : label != null
      ? dayMonth(String(label))
      : "";
  return (
    <div className="rounded-lg border border-border bg-surface px-3 py-2 text-[12px] shadow-md">
      ...
      <div className="font-mono tabular-nums text-accent-deep">
...
function TrendChip({ value }: { value: number }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[11px] font-bold ${up ? "bg-accent-softer text-accent-deep" : "bg-danger-soft text-danger"}`}
    >
      {up ? "▲" : "▼"} {Math.abs(value)}%
    </span>
  );
...
function Kpi({
  ...
  return (
    <div
      className={`rounded-2xl border px-5 py-4 shadow-sm ${accent ? "border-accent/30 bg-accent-softer" : "border-border bg-surface"}`}
    >
      ...
      <div
        className={`mt-1 font-mono text-[26px] font-bold tabular-nums ${accent ? "text-accent-deep" : "text-ink"}`}
      >
        {value}
      </div>
      {hint && (
        <div
          className={`mt-0.5 text-[11.5px] ${hintWarn ? "font-semibold text-warn" : "text-ink-faint"}`}
        >
          {hint}
        </div>
      )}
    ...
    <div className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
      <div className="mb-2 flex items-center gap-2">
        ...
        {badge !== undefined && badge > 0 && (
          <span
            className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${tone === "warn" ? "bg-warn-soft text-warn" : "bg-canvas text-ink-mute"}`}
          >
            {badge}
          </span>
        )}
      ...
      {hint && (
        <p className="-mt-1 mb-2 text-[11.5px] text-ink-faint">{hint}</p>
      )}
      {children}
    </div>
  );
...
function EmptyChart() {
  return (
    <div className="grid h-[260px] place-items-center text-sm text-ink-mute">

...
function amountOf(inv: InvoiceSummary): string {
  return inv.type === "sale"
    ? (inv.sale?.finalAmount ?? "0")
    : (inv.purchase?.totalAmount ?? "0");
}
...
export default function Factures() {
  ...
  async function changeStatus(inv: InvoiceSummary, next: InvoiceStatus) {
    ...
    try {
      ...
      setInvoices((list) =>
        list.map((i) =>
          i.id === inv.id ? { ...i, status: updated.status } : i,
        ),
      );
    } finally {
      setBusyId(null);
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-mute">
          Ventes et achats — téléchargez les PDF, suivez les règlements.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <Input
          label="Recherche"
          icon={searchIcon}
          placeholder="Numéro, client, fournisseur…"
          value={search}
        ...
        <Select
          label="Type"
          value={type}
          onChange={(e) => setType(e.target.value as "all" | InvoiceType)}
        >
              ...
              <tbody>
                {pageItems.map((inv) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-3 font-mono text-[12.5px] font-semibold text-ink tabular-nums">
                        {inv.number}
                      </td>
                      ...
                      <td
                        className={`px-5 py-3 text-right font-mono font-semibold tabular-nums ${inv.status === "cancelled" ? "text-ink-faint line-through" : "text-ink"}`}
                      >
                      ...
                      <td className="px-5 py-3">
                        {isOwner && inv.status !== "cancelled" ? (
                          <select
                            ...
                            onChange={(e) =>
                              changeStatus(inv, e.target.value as InvoiceStatus)
                            }
...
function StatCard({
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface px-5 py-4 shadow-sm">
      ...
      <div
        className={`mt-1 font-mono text-2xl font-bold tabular-nums ${color}`}
      >
        {value}
      </div>
    </div>

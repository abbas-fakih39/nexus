...
export default function NouvelleVente() {
  ...
  const change =
    payment === "cash" && received !== "" ? receivedNum - final : null;
  ...
  function setLineDiscount(id: string, d: number) {
    setCart((c) =>
      c.map((l) =>
        l.product.id === id
          ? { ...l, discount: Math.max(0, Math.min(d, 100)) }
          : l,
      ),
    );
  }
  ...
  const canValidate =
    cart.length > 0 &&
    !submitting &&
    (payment !== "cash" || (received !== "" && receivedNum >= final));
        ...
        <div className="grid grid-cols-2 gap-3 xl:grid-cols-3">
          {filtered.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className="flex flex-col rounded-xl border border-border bg-surface p-3.5 shadow-sm"
              >
      ...
      <aside className="flex h-fit flex-col gap-4 rounded-2xl border border-border bg-surface p-5 shadow-sm lg:sticky lg:top-[88px]">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1.5 flex items-center gap-1.5">
                    ...
                    <input
                      ...
                      value={l.discount || ""}
                      onChange={(e) =>
                        setLineDiscount(l.product.id, Number(e.target.value))
                      }
                      placeholder="0"
              ...
              <button
                ...
                className={`flex flex-col items-center gap-1.5 rounded-xl border py-3 text-[12px] font-semibold transition ${
                  payment === m.value
                    ...
                    : "border-border text-ink-mute hover:bg-canvas"
                }`}
              >
                {m.icon}
                {m.label}
        ...
        {payment === "cash" && (
          <div className="flex flex-col gap-2 rounded-xl bg-canvas p-3">
            ...
            {change !== null && (
              <div
                className={`flex justify-between text-sm font-semibold ${change < 0 ? "text-danger" : "text-accent-deep"}`}
              >
      ...
      <Modal
        ...
        footer={
          <div className="flex w-full items-center justify-between gap-3">
            {success?.invoiceId ? (
              <InvoiceActions invoiceId={success.invoiceId} paid />
            ) : (
              <span />
            )}
        ...
        <div className="flex flex-col gap-2 text-sm">
          ...
          {success?.change != null && (
            <div className="flex justify-between">
              ...
              <span className="font-mono font-bold tabular-nums text-accent-deep">
                {formatEuro(success.change)}
              </span>
            </div>
          )}
        </div>

...
export default function SalesHistory() {
  ...
  async function load() {
    ...
    try {
      setSales(
        await getSales({ from: from || undefined, to: to || undefined }),
      );
    } finally {
      setLoading(false);
    }
                ...
                {pageItems.map((s) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={`px-5 py-3 text-right font-mono font-semibold tabular-nums ${s.status === "cancelled" ? "text-ink-faint line-through" : "text-ink"}`}
                      >
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-0.5 text-[13px] text-ink-mute">
            {PAYMENT_LABEL[detail.paymentMethod]} · Vendu par{" "}
            {detail.soldBy?.name ?? "—"}
          </div>
        </div>
        <Badge tone={badge.tone}>{badge.label}</Badge>
        ...
        {globalDiscount > 0 && (
          <div className="flex justify-between text-ink-mute">
            ...
            <span className="font-mono tabular-nums">
              −
              {formatEuro(
                Number(detail.totalAmount) - Number(detail.finalAmount),
              )}
            </span>
          </div>
        )}
